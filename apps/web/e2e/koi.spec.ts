...
test.describe('SatoshiKoi Asset Lifecycle', () => {
  ...
  test('Connect -> Mint Gen 0 Koi -> Discover Asset', async ({ page }) => {
    ...
    await test.step('Assert SatoshiKoi Pond Section', async () => {
      ...
      await expect(
        page.getByRole('button', { name: /Mint Gen 0 Fish/i }),
      ).toBeVisible();
    });
    ...
    await test.step('Mint Gen 0 Koi', async () => {
      ...
      await expect(page.getByRole('button', { name: /Minting/i })).toBeVisible({
        timeout: 5000,
      });

      // Wait for status message to appear
      await expect(page.getByText(/Fish sent to pool/i)).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    await test.step('Discover Asset in Wallet', async () => {
      ...
      await expect(page.getByText(/Assets \(\d+\)/i)).toBeVisible({
        timeout: 5000,
      });
    });
  });
