...
export function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'swap' | 'koi' | 'watch'>('swap');
  ...
  const handleConfirmSwap = async () => {
    ...
    try {
      ...
      if (chaosMode) {
        ...
        const commitResponse = await commitSwap(
          quote.id,
          l2TxId,
          userL1Address.trim(),
        );
  ...
  const hasInsufficientFunds =
    requiredAmount > 0 && selectedTotal < requiredAmount;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8 w-full overflow-x-hidden">
      <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-2 text-3xl font-bold text-white">
                ...
                <span className="text-xl text-gray-400">ARK</span>
              </p>
              {address && (
        ...
        {/* Tab Navigation */}
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-2 backdrop-blur-sm">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium text-gray-400">
                    Simulate Employer Deposit
                  </h3>
                  <p className="mt-1 text-sm text-gray-500">
                    Initiate a deposit that will be processed in the next Round
                    (5 seconds)
                  </p>
                  {liftStatus && (
                    <p className="mt-2 text-sm text-blue-400 break-words">
                      {liftStatus}
                    </p>
                  )}
                  {faucetError && (
            ...
            <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-6 backdrop-blur-sm">
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-500">
                  Enter an amount to request a swap quote and generate the lock
                  address
                </p>
                  ...
                  <>
                    ...
                    {hasInsufficientFunds && (
                      <p className="text-sm text-yellow-400">
                        Insufficient funds selected:{' '}
                        {selectedTotal.toLocaleString()} /{' '}
                        {requiredAmount.toLocaleString()} sats
                      </p>
                    )}
                    {quoteError && (
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    if (
                                      ...
                                      try {
                                        const selected =
                                          mockArkClient.selectCoins(
                                            address,
                                            amount,
                                          );
                                        setSelectedVtxos(
                                          selected.map((v) => v.txid),
                                        );
                                      } catch (error) {
                                        console.error(
                                          'Auto coin selection reset failed',
                                          error,
                                        );
                                        setSelectedVtxos([]);
                                      }
                    ...
                    {lockAddress && quote && (
                      <div className="space-y-4">
                        <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-4 overflow-hidden">
                          ...
                          <p className="text-sm font-mono text-green-400 break-all overflow-wrap-anywhere">
                            {lockAddress}
                          </p>
                        </div>
                        <div className="space-y-2">
                  ...
                  <div className="flex flex-col items-center justify-center py-8">
                    ...
                    {!loadingText && (
                      <p className="text-xs text-gray-500 mt-2">
                        Locking funds and executing payout
                      </p>
                    )}
                  </div>
                )}
                        ...
                        <div className="flex flex-col items-center justify-center py-8">
                          ...
                          {currentBlock !== null && timeoutBlock !== null && (
                            <div className="mt-4 space-y-2 text-center">
                              ...
                              <p className="text-xs text-gray-500 mt-2">
                                Blocks remaining:{' '}
                                {Math.max(0, timeoutBlock - currentBlock)}
                              </p>
                            </div>
                          )}
                          {(currentBlock === null || timeoutBlock === null) && (
                        ...
                        {currentBlock !== null && timeoutBlock !== null && (
                          <>
                            <button
                              ...
                              disabled={
                                isClaimingRefund || currentBlock < timeoutBlock
                              }
                              title={
